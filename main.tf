...
resource "google_storage_bucket" "ora_bucket" {
  name     = var.oraname
...
resource "google_storage_bucket_object" "ora_object1" {
  name   = var.file1
  bucket = google_storage_bucket.ora_bucket.name
  source = var.s1
}
...
resource "google_storage_bucket" "sqlautobackuptostrg_bucket" {
  name     = var.sqlname
  location = var.location
  storage_class = var.storage_class
}
...
resource "google_storage_bucket_object" "sql_object1" {
  name   = var.sqlfile1
  bucket = google_storage_bucket.sqlautobackuptostrg_bucket.name
  source = var.sql1
}
